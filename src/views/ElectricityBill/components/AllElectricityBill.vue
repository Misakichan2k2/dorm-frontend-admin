...
import { STORE_ELECTRIC_INVOICE } from "@/services/stores";
...
} = STORE_ELECTRIC_INVOICE.StoreElectricInvoice();
...
const fetchElectricInvoices = async () => {
  await onActionGetAllElectricInvoices().then((res) => {
    console.log(res);
...
const saveNewInvoice = async () => {
  ...
  try {
    await onActionCreateElectricInvoice({
      room: roomId,
      month,
      year,
...
const saveEditedInvoice = async () => {
  try {
    ...
    await onActionUpdateElectricInvoice(selectedInvoice.value._id, {
...
const handleExport = () => {
  const data = filteredInvoices?.value.map((item, index) => ({
    ...
    "Đơn giá 1KWH (VNĐ)": item.unitPrice,
    "Thành tiền (VNĐ)": (
      item.unitPrice *
  ...
  exportToExcel(data, "danh-sach-hoa-don-dien");
};
        ...
        <v-container>
          ...
          <v-row dense>
            <v-col cols="6">
              ...
              {{ selectedInvoice.totalConsumption }} kWh
            </v-col>
            <v-col cols="6">
              <strong>Đơn giá:</strong>
              {{ selectedInvoice.unitPrice.toLocaleString("vi-VN") }} đ/kWh
            </v-col>
          </v-row>
  ...
  <v-dialog v-model="dialogEditInvoice" max-width="600px">
    <v-card>
      ...
      <v-card-actions class="pa-2">
        <v-spacer />
        <v-btn color="grey" text @click="dialogEditInvoice = false">Hủy</v-btn>
